...
struct TexturePackPathes {
    std::string walls_texture_pack="content/texture_packs/map/tiles1";  
    std::string tank_texture_pack_="content/texture_packs/tank/tank1";
};
...
private:
    ...
    TankTexturePack tank_texture_pack_;
    TileTexturePack tile_texture_pack_;

    std::vector<Explosion> explosions_;
    raii::SDL_Texture explosion_sheet_;

    int explosion_frame_width_ = 0;
    int explosion_frame_height_ = 0;
...
public:
    Graphics(const Config &config) : config_(config) {
        ...
        tank_texture_pack_.load(renderer_.get(), config.texture_pack_pathes.tank_texture_pack_);
        tile_texture_pack_.load(renderer_.get(), config.texture_pack_pathes.walls_texture_pack);
    }
...
private:
    ...
    void render_tank(const Tank &tank) {     
        SDL_SetRenderTarget(renderer_.get(), tank_texture_pack_.tank_base.get());
        SDL_SetRenderDrawColor(renderer_.get(), 0, 0, 0, 0); 
        SDL_RenderClear(renderer_.get());

        // Body
        SDL_Point tank_texture_size;
        SDL_QueryTexture(tank_texture_pack_.body.get(), nullptr, nullptr, &tank_texture_size.x, &tank_texture_size.y);
        SDL_Rect tank_texture_rect = {0, 0, tank_texture_size.x, tank_texture_size.y};
        SDL_RenderCopy(renderer_.get(), tank_texture_pack_.body.get(), nullptr, &tank_texture_rect);

        // Turret
        SDL_Point turret_texture_size;
        SDL_QueryTexture(tank_texture_pack_.turret.get(), nullptr, nullptr, &turret_texture_size.x, &turret_texture_size.y);
        SDL_Rect turret_texture_rect = { 
            tank_texture_pack_.turret_slot_center.x - tank_texture_pack_.turret_center.x,
            tank_texture_pack_.turret_slot_center.y - tank_texture_pack_.turret_center.y,
            turret_texture_size.x,
            turret_texture_size.y
        };
        SDL_RenderCopy(renderer_.get(), tank_texture_pack_.turret.get(), nullptr, &turret_texture_rect);
        ...
        SDL_RenderCopyEx(renderer_.get(),
                        tank_texture_pack_.tank_base.get(),
                        nullptr,
                        &tank_rect,
                        angle,
                        &center,
